...
post_commit_dir = '/config/scripts/commit/post-hooks.d'
...
def verify(config):
    ...
    for option in ['primary', 'secondary', 'section', 'mode']:
        if option not in config:
            raise ConfigError(f"config-sync '{option}' is not configured!")

    for router in ['primary', 'secondary']:
        if 'address' not in config[router]:
            raise ConfigError(f'{router} address is mandatory!')
        if 'key' not in config[router]:
            raise ConfigError(f'{router} key is mandatory!')
