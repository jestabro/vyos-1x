...
static char * get_recv_string (void *, int);
...
//    zmq_send(requester, "send", 4, 0);
    ...
    char *out = get_recv_string(requester, msg_size);
    if (out != NULL) {
        printf("%s", out);
    }
...
static char * get_recv_string (void *socket, int buf_size) {
    char * buffer;

    if (buf_size == 0) {
        printf("JSE yes buf_size is 0\n");
        return NULL;
    } else {
        printf("JSE no buf_size is not 0: %d\n", buf_size);
        buffer = (char *) malloc(buf_size);
    }

    zmq_send(socket, "send", 4, 0);
