...
"""
frozen_config.py
This is a modification of config.py with an alternate constructor, to be
used for 'offline' (out of config session) processing of conf_mode scripts.
Original config.py comments below this line.
...
class FrozenConfig(object):
    """
    The class of frozen config access objects.
    ...
    subtrees.
    """
    def __init__(self, session_env=None, running_config_text=None,
                                         session_config_text=None):
        self._level = []
        if session_env:
    ...
    def get_config_dict(self, path=[], effective=False, key_mangling=None):
        ...
        """
        # --- XXX --- this still needs to made relative to path by returning
        # sub-dict of full dict.

        config_dict = {}

        if effective:
            if self._running_config:
                if not path or self._running_config.exists(self._make_path(path)):
